...
with open("NaivBay.yaml", "r") as file:
...
def train():
    ...
    if config.model == 'multinomialNB':
        model = MultinomialNB()
        model.alpha = config.alpha
        model.fit_prior = config.fit_prior
        # model.class_prior = config.class_prior
    elif config.model == 'complementNB':
        model = ComplementNB()
        model.alpha = config.alpha
        model.fit_prior = config.fit_prior
        # model.class_prior = config.class_prior
        model.norm = False
    elif config.model == 'complementNB_norm_True':
        model = ComplementNB()
        model.alpha = config.alpha
        model.fit_prior = config.fit_prior
        # model.class_prior = config.class_prior
